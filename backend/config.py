...
# Enrichment can confirm a tmdb_id on a movie/series row that turns out to
# match an existing row's tmdb_id exactly -- a signal that comes from TMDB
# itself, not from our own name-normalization heuristics, so it's trusted
# enough to merge without a human click (see vod_db.auto_merge_movie_by_tmdb /
# auto_merge_series_by_tmdb). Default ON, unlike the quality-prefix flag
# above -- that flag changes what gets SUGGESTED for manual review (low
# stakes to leave on), this flag changes what gets MERGED automatically (an
# irreversible delete), so the two defaults look inconsistent but are
# deliberately opposite: this one is gated on an independently-corroborated
# exact-id match, which is a strong enough signal to default-enable even
# though the action itself is destructive.
