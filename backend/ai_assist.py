"""
AI-assisted category creation and Needs Review disambiguation, via the
Anthropic Messages API. Two distinct capabilities for categories, matched to
two very different cost/reliability profiles, plus a third for Needs Review:

- Light mode (suggest_category_rule): one Claude call translates a plain-
...
- Heavy mode (evaluate_candidates_for_category): for criteria the rule
  ...
  audience fit -- there's no "keyword" or "cast" field), Claude judges
...
- suggest_year_review_match: picks the most likely correct match among TMDB
  search candidates already fetched for one Needs Review item, with
  reasoning. Always a suggestion the reviewer still has to click to accept
  (see vod_routes.py's /needs-review/.../resolve/) -- nothing here ever
  resolves a match on its own.
"""
...
async def _call_claude(system: str, user_message: str, tool_name: str, tool_description: str, schema: dict, max_tokens: int = 1024) -> dict:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        r = await client.post(_API_BASE, json=body, headers={
            "x-api-key": api_key,
